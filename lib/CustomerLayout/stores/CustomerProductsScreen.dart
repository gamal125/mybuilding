...
class CustomerProductsScreen extends StatefulWidget {
  const CustomerProductsScreen({super.key,required this.id,required this.storeName,required this.phone});
final String id;
...
ids={doc['id']:doc.id};
                ...
                GridView.count(
                    ...
                    children:searchController.text.isEmpty? List.generate(
                        productModels.length, (index) => productWidget(productModels[index],)):List.generate(
                        filesSearch.length, (index) => productWidget(filesSearch[index]))
                ):const SizedBox();
              }
  ...
  Widget productWidget(ProductModel model)=> InkWell(
    onTap: (){
    ...
    ids.forEach((key, value) {
      if(key==model.id){
        ...
        navigateTo(context, ProductDetailsScreen(model: model, id:value, tenanted: false,));
